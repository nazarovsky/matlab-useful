...

{
  ...
  if (nrhs != 1) {
    mexErrMsgIdAndTxt("MATLAB:mex_flicker:nargin", 
            " requires three input arguments.");
  } else if (nlhs > 1) {
    mexErrMsgIdAndTxt("MATLAB:mex_flicker:nargout",
    ...
    if( !mxIsDouble(prhs[0]) || 
         mxIsComplex(prhs[0])) {
        mexErrMsgIdAndTxt("MATLAB:mex_flicker:notDouble","input vector must be type double.");
    }
